...
type GroupPointersEncoding struct {
	...
	// field UserUse is a slice of pointers to another Struct (optional or 0..1)
	UserUse IntSlice `gorm:"type:TEXT"`
}
...
func (backRepoGroup *BackRepoGroupStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, group *models.Group) (Error error) {
	...
	if groupDB, ok := backRepoGroup.Map_GroupDBID_GroupDB[idx]; ok {
		...
		// 1. reset
		groupDB.GroupPointersEncoding.UserUse = make([]int, 0)
		// 2. encode
...
func (groupDB *GroupDB) DecodePointers(backRepo *BackRepoStruct, group *models.Group) {
	...
	// This loop redeem group.UserUse in the stage from the encode in the back repo
	// It parses all UserUseDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	group.UserUse = group.UserUse[:0]
	for _, _UserUseid := range groupDB.GroupPointersEncoding.UserUse {
		group.UserUse = append(group.UserUse, backRepo.BackRepoUserUse.Map_UserUseDBID_UserUsePtr[uint(_UserUseid)])
	}
